...
public class Algorithms {
	...
	public static <T extends RealType<T> & NativeType<T>> RealPoint getMaximumLocation(RandomAccessibleInterval<T> rai,
			double[] calibration) {
		return getMaximumLocation(Views.iterable(rai), calibration);
	}

	public static <T extends RealType<T> & NativeType<T>> RealPoint getMaximumLocation(IterableInterval<T> ii,
			double[] calibration) {
	...
	public static <T extends RealType<T>> Double getMaximumValue(RandomAccessibleInterval<T> rai) {
	...
	public static <T extends RealType<T>> Pair<Double, Double> getMinMaxValues(RandomAccessibleInterval<T> rai) {
	...
	public static <T extends RealType<T> & NativeType<T>> boolean isCenterLargest(T center,
	...
	public static <T extends RealType<T> & NativeType<T>> List<RealPoint> findLocalMaximumValues(
			RandomAccessibleInterval<T> rai, Shape shape) {
		...
		LoopBuilder.setImages(neighborhoodsInterval, rai).forEachPixel((neighborhood, center) -> {
	...
	public static Set<Integer> getCentralLabels(ImgLabeling<Integer, IntType> labeling, double[] center,
		...
		final RandomAccessible<Neighborhood<IntType>> nra = sphere
				.neighborhoodsRandomAccessible(Views.extendZero(labeling.getIndexImg()));
		...
		for (int index : centralIndices) {
			final ArrayList<Integer> labels = new ArrayList<>(labeling.getMapping().labelsAtIndex(index));
	...
	public static Img<UnsignedByteType> createUnsignedByteTypeMaskFromLabelRegion(
	...
	public static <T extends RealType<T> & NativeType<T>> void copy(RandomAccessibleInterval<T> source,
	...
	public static <T extends RealType<T> & NativeType<T>> RandomAccessibleInterval<BitType> createMask(
			RandomAccessibleInterval<T> rai, double threshold) {
		Logger.log("Computing mask...");
		RandomAccessibleInterval<BitType> mask = Converters.convert(rai,
				(i, o) -> o.set(i.getRealDouble() > threshold ? true : false), new BitType());

		return Utils.copyAsArrayImg(mask);
	}

	public static <T extends RealType<T> & NativeType<T>> RandomAccessibleInterval<BitType> fillHoles3Din2D(
			RandomAccessibleInterval<BitType> mask, int axis, OpService opService) {
	...
	public static <T extends RealType<T> & NativeType<T>> RandomAccessibleInterval<BitType> createWatershedSeeds(
			RandomAccessibleInterval<T> distance, Shape shape, double globalThreshold, double localThreshold) {
		...
		RandomAccessible<Neighborhood<T>> neighborhoods = shape
				.neighborhoodsRandomAccessible(Views.extendPeriodic(distance));
		...
		while (neighborhoodCursor.hasNext()) {
			...
			} else if (Utils.isLateralBoundaryPixel(neighborhood, distance)
					&& distanceRandomAccess.get().getRealDouble() > 0) {
				seedsRandomAccess.get().set(true);
	...
	public static <T extends RealType<T> & NativeType<T>> RandomAccessibleInterval<BitType> createLocalMaximaMask(
			RandomAccessibleInterval<T> input, Shape shape, double minimumValue) {
		...
		RandomAccessible<Neighborhood<T>> neighborhoods = shape
				.neighborhoodsRandomAccessible(Views.extendPeriodic(input));
	...
	public static <R extends RealType<R> & NativeType<R>> RandomAccessibleInterval<R> computeGradient(
			RandomAccessibleInterval<R> input, Shape shape) {

		RandomAccessible<Neighborhood<R>> neighborhoods = shape
				.neighborhoodsRandomAccessible(Views.extendBorder(input));
	...
	public static <T extends RealType<T> & NativeType<T>> RandomAccessibleInterval<BitType> createCenterAndBoundarySeeds(
			RandomAccessibleInterval<T> distance, Shape shape, double globalThreshold, double localThreshold) {
...
//			}
	...
	public static <T extends RealType<T> & NativeType<T>> ArrayList<PositionAndValue> getLocalMaxima(
			RandomAccessibleInterval<T> rai, double minimalDistanceBetweenMaxima, double threshold) {
		Shape shape = new HyperSphereShape((long) minimalDistanceBetweenMaxima);
		RandomAccessible<Neighborhood<T>> neighborhoods = shape
				.neighborhoodsRandomAccessible(Views.extendPeriodic(rai));
	...
	private static <T extends RealType<T> & NativeType<T>> boolean isCenterLargestOrEqual(T center,
			Neighborhood<T> neighborhood) {
		for (T neighbor : neighborhood) {
			if (neighbor.compareTo(center) > 0) {
				return false;
	...
	private static <T extends RealType<T> & NativeType<T>> double computeMinimum(Neighborhood<T> neighborhood) {
	...
	public static <T extends RealType<T> & NativeType<T>> void splitCurrentObjectsBasedOnOverlapWithPreviousObjects(
			RandomAccessibleInterval<BitType> outputMask,
			HashMap<Integer, ArrayList<Integer>> overlappingObjectsLabelsMap,
			ImgLabeling<Integer, IntType> currentImgLabeling, RandomAccessibleInterval<T> currentIntensities,
			RandomAccessibleInterval<IntType> previousLabeling, long minimalObjectSize, long minimalObjectWidth,
			OpService opService, boolean showSplits) {
		...
		for (int currentObjectLabel : overlappingObjectsLabelsMap.keySet()) {
			final ArrayList<Integer> overlappingPreviousObjectLabels = overlappingObjectsLabelsMap
					.get(currentObjectLabel);

			if (overlappingPreviousObjectLabels.size() > 1) {
				RandomAccessibleInterval<BitType> currentObjectMask = Regions
						.asMask(currentRegions.getLabelRegion(currentObjectLabel));
				RandomAccessibleInterval<IntType> previousLabelingCrop = Views.interval(previousLabeling,
						currentObjectMask);

				currentObjectMask = Views.zeroMin(currentObjectMask);
				previousLabelingCrop = Views.zeroMin(previousLabelingCrop);

				final RandomAccessibleInterval<T> maskedAndCroppedIntensities = Views.zeroMin(Regions
						.getMaskedAndCropped(currentIntensities, currentRegions.getLabelRegion(currentObjectLabel)));

				final RandomAccessibleInterval<IntType> overlapLabeling = createOverlapLabeling(currentObjectMask,
						previousLabelingCrop, overlappingPreviousObjectLabels);

				final Set<Long> uniqueValues = Utils.computeUniqueValues(overlapLabeling);

				if (uniqueValues.size() <= 2) {
					// "<=2" because it includes 0, thus for two objects there should be at least 3
					// unique values
...
//								localMaxima );
				...
				final ImgLabeling<Integer, IntType> seedsImgLabeling = createImgLabelingFromLabeling(
						overlappingPreviousObjectLabels, overlapLabeling);

				final RandomAccessibleInterval<T> invertedView = Utils.invertedView(maskedAndCroppedIntensities);

				opService.image().watershed(watershedImgLabeling, invertedView, seedsImgLabeling, true, true,
						currentObjectMask);
				...
				if (splitObjects.getExistingLabels().contains(-1)) {
					...
					// sometimes the watershed is weirdly placed such that very small (single pixel)
					// objects can occur
	...
	public static ImgLabeling<Integer, IntType> createImgLabelingFromLabeling(
			ArrayList<Integer> overlappingPreviousObjectLabels, RandomAccessibleInterval<IntType> seeds) {
	...
	public static <T extends RealType<T> & NativeType<T>> void splitTouchingObjects(
			ImgLabeling<Integer, IntType> imgLabeling, RandomAccessibleInterval<T> intensity,
			RandomAccessibleInterval<BitType> mask, // This will be changed, i.e. the split(s) will be drawn into it
			HashMap<Integer, Integer> numObjectsPerRegion, long minimalObjectWidth, long minimalObjectSize,
			long maximalWatershedBoundaryLength, OpService opService, boolean forceSplit,
		...
		for (int label : numObjectsPerRegion.keySet()) {
			if (numObjectsPerRegion.get(label) > 1) {
				...
				final RandomAccessibleInterval<T> maskedAndCroppedIntensities = Views
						.zeroMin(Regions.getMaskedAndCropped(intensity, labelRegions.getLabelRegion(label)));
				final RandomAccessibleInterval<BitType> labelRegionMask = Views
						.zeroMin(Regions.asMask(labelRegions.getLabelRegion(label)));

				final ArrayList<PositionAndValue> localMaxima = computeSortedLocalIntensityMaxima(
						2 * minimalObjectWidth, maskedAndCroppedIntensities, showSplittingAttempts);
				...
				final RandomAccessibleInterval<BitType> seeds = positionsAsBinaryImage(numObjectsPerRegion.get(label),
						maskedAndCroppedIntensities, localMaxima);

				final ImgLabeling<Integer, IntType> watershedImgLabeling = createEmptyImgLabeling(labelRegionMask);
				final ImgLabeling<Integer, IntType> seedsImgLabeling = Regions.asImgLabeling(seeds,
						ConnectedComponents.StructuringElement.FOUR_CONNECTED);

				opService.image().watershed(watershedImgLabeling, Utils.invertedView(maskedAndCroppedIntensities),
						seedsImgLabeling, true, true, labelRegionMask);
				...
				if (forceSplit) {
					...
					isValidSplit = checkSplittingValidity(splitObjects, minimalObjectSize,
	...
	public static <T extends RealType<T> & NativeType<T>> RandomAccessibleInterval<BitType> createObjectSkeletons(
			ImgLabeling<Integer, IntType> imgLabeling, int closingRadius, OpService opService) {
		...
		for (LabelRegion<IntType> labelRegion : labelRegions) {
			RandomAccessibleInterval<BitType> labelRegionMask = Views.zeroMin(Regions.asMask(labelRegion));
	...
	public static <T extends RealType<T> & NativeType<T>> ArrayList<PositionAndValue> computeSortedLocalIntensityMaxima(
			long minimalObjectWidth, RandomAccessibleInterval<T> maskedAndCropped, boolean showSplittingAttempts) {
		...
		final ArrayList<PositionAndValue> sortedLocalMaxima = Algorithms.getLocalMaxima(blurred, minimalObjectWidth,
				0.0);

		return sortedLocalMaxima;
	}

	public static <I extends IntegerType<I>> RandomAccessibleInterval<IntType> createOverlapLabeling(
			RandomAccessibleInterval<BitType> currentObjectMask, RandomAccessibleInterval<I> previousLabeling,
			ArrayList<Integer> previousLabels) {
		RandomAccessibleInterval<IntType> overlapLabeling = ArrayImgs
				.ints(Intervals.dimensionsAsLongArray(currentObjectMask));
...
//		ImageJFunctions.show( overlapLabeling, "overlap labeling" );
		...
		// below is necessary because watershed seeds are not allowed to touch the mask
		// boundary
	...
	private static <T extends RealType<T> & NativeType<T>> RandomAccessibleInterval<BitType> positionsAsBinaryImage(
			int numPositions, RandomAccessibleInterval<T> maskedAndCropped, ArrayList<PositionAndValue> positions) {
		RandomAccessibleInterval<BitType> binaryImage = ArrayImgs
				.bits(Intervals.dimensionsAsLongArray(maskedAndCropped));
	...
	public static boolean checkSplittingValidity(LabelRegions<Integer> splitObjects, long minimumObjectSize,
	...
	public static boolean isWatershedValid(LabelRegions<Integer> splitObjects, long maximalWatershedLength) {
		...
		for (LabelRegion region : splitObjects) {
			...
			if (splitObjectLabel == WATERSHED) {
				final ImgLabeling<Integer, IntType> imgLabeling = Regions.asImgLabeling(Regions.asMask(region),
						ConnectedComponents.StructuringElement.FOUR_CONNECTED);
	...
	public static void drawSkeleton(RandomAccessibleInterval<BitType> output,
			RandomAccessibleInterval<BitType> skeleton, long[] regionOffset) {
	...
	public static void drawWatershedIntoMask(RandomAccessibleInterval<BitType> mask, LabelRegions labelRegions,
	...
	public static RandomAccessibleInterval<BitType> close(RandomAccessibleInterval<BitType> mask, int closingRadius) {
		...
		// TODO: Bug(?!) in imglib2 Closing.close makes enlargement necessary, otherwise
		// one gets weird results at boundaries
	...
	public static RandomAccessibleInterval<BitType> open(RandomAccessibleInterval<BitType> mask, int radius) {
		if (radius <= 0)
			return mask;
		...
		final RandomAccessibleInterval<BitType> enlargedMask = Utils.getEnlargedRai(mask, radius);
		final RandomAccessibleInterval<BitType> enlargedMorphed = Utils.getEnlargedRai(morphed, radius);
	...
	public static <T extends RealType<T> & NativeType<T>> ArrayList<RandomAccessibleInterval<T>> createMaximumIntensityProjectionsAssumingImagePlusDimensionOrder(
			RandomAccessibleInterval<T> inputImages, long channel, long tMin, long tMax) {
	...
	public static RandomAccessibleInterval<DoubleType> computeSquaredDistances(RandomAccessibleInterval<BitType> mask) {
		final RandomAccessibleInterval<DoubleType> doubleBinary = Converters.convert(mask,
				(i, o) -> o.set(i.get() ? Double.MAX_VALUE : 0), new DoubleType());

		RandomAccessibleInterval<DoubleType> distance = ArrayImgs
				.doubles(Intervals.dimensionsAsLongArray(doubleBinary));
	...
	public static <R extends RealType<R> & NativeType<R>> RandomAccessibleInterval<R> erode(
			RandomAccessibleInterval<R> image, int radiusPixels) {
	...
	public static <R extends RealType<R> & NativeType<R>> RandomAccessibleInterval<R> dilate(
			RandomAccessibleInterval<R> image, int radius) {
		final RandomAccessibleInterval<R> morphed = Utils.createEmptyCopy(image);
	...
	public static <R extends RealType<R> & NativeType<R>> RandomAccessibleInterval<R> median(
			RandomAccessibleInterval<R> intensity, int radius, OpService opService) {
		RandomAccessibleInterval<R> median = Utils.createEmptyCopy(intensity);

		opService.filter().median(Views.iterable(Views.zeroMin(median)), Views.zeroMin(intensity),
	...
	public static <R extends RealType<R> & NativeType<R>> RandomAccessibleInterval<R> median2dUsingIJ1(
			RandomAccessibleInterval<R> intensity, int radius) {
	...
	public static <T extends RealType<T> & NativeType<T>> CoordinatesAndValues computeRadialProfile(
			RandomAccessibleInterval<T> image, double[] center, double spacing, double maxDistanceInMicrometer) {
	...
	public static <T extends RealType<T>> Histogram1d<T> histogram(RandomAccessibleInterval<T> rai, int numBins) {
		final Pair<Double, Double> minMaxValues = getMinMaxValues(rai);

		final Real1dBinMapper<T> tReal1dBinMapper = new Real1dBinMapper<>(minMaxValues.getA(), minMaxValues.getB(),
				numBins, false);
	...
	public static <T extends RealType<T>> double thresholdOtsu(RandomAccessibleInterval<T> rai) {
	...
	public static <T extends RealType<T>> double thresholdHuang(RandomAccessibleInterval<T> rai) {
		final Histogram1d<T> histogram = histogram(rai, 256);
	...
	public static <T extends RealType<T>> double thresholdYen(RandomAccessibleInterval<T> rai) {
		final Histogram1d<T> histogram = histogram(rai, 256);
		final T type = Views.iterable(rai).firstElement().createVariable();
