...
public abstract class TransformConversions {
	public static AffineTransform3D getAmiraAsPixelUnitsAffineTransform3D(double[] amiraRotationAxis,
			double amiraRotationAngleInDegrees, double[] amiraTranslationVectorInMicrometer,
			double[] targetImageVoxelSizeInMicrometer, double[] targetImageCenterInPixels // this is the center of the
																							// rotation
	) {

		// rotate
		//
		final Vector3D axis = new Vector3D(amiraRotationAxis[0], amiraRotationAxis[1], amiraRotationAxis[2]);
		...
		final AffineTransform3D transform3D = getRotationAroundImageCenterTransform(rotationTransform,
	...
	public static String get_DOESNOTWORK_USE_INVERSE_AmiraAsElastixAffine3D(double[] amiraRotationAxis,
			double amiraRotationAngleInDegrees, double[] amiraTranslationVectorInMicrometer,
			double targetImageVoxelSizeInMicrometer, double[] rotationCentreInPixels) // this is the center of the
																						// rotation)
	{
	...
	public static AffineTransform3D getRotationAroundImageCenterTransform(AffineTransform3D rotationTransform,
	...
	public static String asStringElastixStyle(AffineTransform3D affineTransform3D, double voxelSizeInMillimeter) {
	...
	public static void changeTransformToScaledUnits(AffineTransform3D affineTransform3D,
			double[] voxelSizeInMicrometer) {
	...
	public static AffineTransform3D getElastixSimilarityAsBdvAffine() {
		...
		// (TransformParameters -0.008415 0.004752 -0.001727 -0.002337 -0.001490
		// 0.003296 0.987273)
		// (InitialTransformParametersFileName
		// "/Users/tischer/Documents/rachel-mellwig-em-prospr-registration/transformations/prospr-to-segmentation/TransformParameters.ManualPreAlignment-Affine_actuallyOnlyEuler.txt")
		...
		// (Direction 1.0000000000 0.0000000000 0.0000000000 0.0000000000 1.0000000000
	...
	public static AffineTransform3D getElastixEulerTransformAsAffineTransformInPixelUnits(String transform,
			String rotation, double[] imageVoxelSizeInMicrometer) {
		...
		for (int d = 0; d < 3; ++d) {
			rotationCentreVectorInPixelsPositive[d] = Double.parseDouble(split[d])
					/ (0.001 * imageVoxelSizeInMicrometer[d]);
			rotationCentreVectorInPixelsNegative[d] = -Double.parseDouble(split[d])
					/ (0.001 * imageVoxelSizeInMicrometer[d]);
		}
