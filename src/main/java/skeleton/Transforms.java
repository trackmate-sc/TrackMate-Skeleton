...
public abstract class Transforms<T extends InvertibleRealTransform & Concatenable<T> & PreConcatenable<T>> {
	...
	public static AffineTransform3D rotationAroundCenterTransform(double angle, int axis, double[] center) {
	...
	public static <T extends InvertibleRealTransform & Concatenable<T> & PreConcatenable<T>> RealTransform createIdentityAffineTransformation(
			int numDimensions) {
		if (numDimensions == 2) {
	...
	public static <T extends NumericType<T> & NativeType<T>> RandomAccessibleInterval createTransformedView(
			RandomAccessibleInterval<T> rai, InvertibleRealTransform combinedTransform,
	...
	public static <T extends NumericType<T> & NativeType<T>> RandomAccessibleInterval createTransformedView(
		...
		final RandomAccessible transformedRA = createTransformedRaView(rai, transform,
	...
	public static <T extends Type<T>> RandomAccessibleInterval<T> getWithAdjustedOrigin(Interval interval,
	...
	public static <T extends NumericType<T>> RandomAccessible createTransformedRaView(RandomAccessibleInterval<T> rai,
			InvertibleRealTransform combinedTransform, InterpolatorFactory interpolatorFactory) {
	...
	public static FinalInterval createBoundingIntervalAfterTransformation(Interval interval,
	...
	public static AffineTransform3D getTransformToIsotropicRegistrationResolution(double binning,
			double[] calibration) {
	...
	public static <T extends RealType<T> & NativeType<T>> ArrayList<RandomAccessibleInterval<T>> transformAllChannels(
			RandomAccessibleInterval<T> images, AffineTransform3D registrationTransform,
	...
	public static <T extends NumericType<T> & NativeType<T>> RandomAccessibleInterval createTransformedView(
			RandomAccessibleInterval<T> rai, InvertibleRealTransform transform, FinalInterval interval) {
		final RandomAccessible transformedRA = createTransformedRaView(rai, transform,
	...
	public static <T extends NumericType<T>> FinalInterval createTransformedInterval(RandomAccessibleInterval<T> rai,
			InvertibleRealTransform transform) {
	...
	public static <T extends NumericType<T> & NativeType<T>> RandomAccessibleInterval createTransformedView(
			RandomAccessibleInterval<T> rai, InvertibleRealTransform combinedTransform, FinalInterval interval,
			InterpolatorFactory interpolatorFactory) {
		final RandomAccessible transformedRA = createTransformedRaView(rai, combinedTransform, interpolatorFactory);
		final RandomAccessibleInterval<T> transformedIntervalView = Views.interval(transformedRA, interval);
	...
	public static <T extends RealType<T> & NativeType<T>> RandomAccessibleInterval<T> transformAllChannels(
			RandomAccessibleInterval<T> images, AffineTransform3D registrationTransform) {
		ArrayList<RandomAccessibleInterval<T>> transformedChannels = new ArrayList<>();
	...
	public static <T extends RealType<T> & NativeType<T>> RandomAccessibleInterval<T> scaledView(
			RandomAccessibleInterval<T> input, double[] scalingFactors) {
		// Convert to RealRandomAccessible such that we can obtain values at (infinite)
		// non-integer coordinates
		RealRandomAccessible<T> rra = Views.interpolate(Views.extendBorder(input), new NLinearInterpolatorFactory<>());

		// Change scale such that we can sample from integer coordinates (for raster
		// function below)
	...
	public static <T extends RealType<T> & NativeType<T>> double[] getCenter(RealInterval rai) {
	 ...
	 */
	public static AffineTransform3D getRotationTransform3D(double[] normalisedTargetAxis, double[] normalisedAxis) {
		...
		if (LinAlgHelpers.length(rotationAxis) == 0.0) {
			// Since the rotation angle is not zero (see above), the vectors are
			// anti-parallel.
			// This means that the cross product does not work for finding a perpendicular
			// vector.
	...
	public static AffineTransform3D rotationAroundIntervalCenterTransform(double angle, int axis,
